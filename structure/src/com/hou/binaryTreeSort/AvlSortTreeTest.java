package com.hou.binaryTreeSort;
...
public class AvlSortTreeTest {
    ...
    public void addTest() {
        ...
        int[] arr = {1,2,3,4,5,6,7,8,-1,-7};
        for (int i = 0; i < arr.length; i++) {
            tree.add(new Node(arr[i]));
        }
        tree.infixOrder();
        System.out.println("树的总层数是:"+ tree.root.Length());
        System.out.println("左子树的总层数是:"+tree.root.leftLength());
        System.out.println("右子树的总层数是:"+tree.root.rightLength());
...
class Node {
    ...
    public void add(Node node) {
        ...
        //左旋转 如果右边的层数大于左边的层数，这个时候需要对树进行平衡
        if ( rightLength() -leftLength() >1){
            if (right !=null && right.leftLength() - right.rightLength() >1 ){
                // 先对右子树进行右旋
                right.rightRotating();
                // 再对整个树左旋
                leftRotating();
        ...
        if( leftLength() - rightLength() >1){
            //现在要判断根节点的左子树的右子树是否高于左子树
            if (right !=null &&left.rightLength() - left.leftLength() >1){
                //这个时候就要先对左子树进行左旋，再对整个树右旋
                left.leftRotating();
                rightRotating();
            }else {
                rightRotating();
    ...
    public int layer(){
        return Math.max(
                (left == null? 0:left.layer()),
                (right ==null? 0:right.layer())
        )+1;
    }
     ...
     * 左旋转的方法
     */
    public void leftRotating(){
        //第一步先创建一个节点，就是从哪里开始旋转的节点
        Node newNode = new Node(value);
        //第二步把这个节点的左子树设置为原来这个节点的左子树(不变)
        newNode.left = left;
        //第三步把这个节点的右子树设置为原来这个节点的右子树的左子树(把一个节点从右子树哪里拿了过来)
        newNode.right = right.left;
        //第四步 把当前节点的值换为当前节点右子树的值
        value = right.value;
        //第五步 把当前节点右子树变成当前节点右子树的右子树(把右子树的节点抛弃了)
        right = right.right;
        //第六步 把当前节点的左子树 设置为刚刚创建的新节点
        left = newNode;
    }

    /**
     *  右旋转的方法  当左子树的高度大于右子树的时候，使用这个方法
     */
    public void rightRotating(){
        //第一步 把新节点的值设置为根节点的值
        Node newNode = new Node(value);
        //第二步 把新节点的右子树设置为当前节点右子树
        newNode.right = right;
        //第三步 把新节点的左子树设置为当前节点的左子树的右子树
        newNode.left = left.right;
        //第四步 把当前节点的值设置为左子树的值
        value = left.value;
        //第五步 把当前节点的左子树设置为当前节点的左子树的左子树(this.left 被抛弃了)
        left = left.left;
        //第五步  把当前节点的右子树设置为新的节点
        right = newNode;
    }
}